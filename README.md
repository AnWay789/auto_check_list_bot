...
Бот получает список дашбордов для проверки от [Django приложения](https://github.com/AnWay789/auto_check_list_web) через FastAPI, отправляет уведомления в Telegram с кнопками для подтверждения проверки, отслеживает переходы по ссылкам и отправляет результаты обратно в Django.

## Быстрый старт

Переменные окружения задаются в **общем `.env` в родительской директории** (каталог, содержащий оба проекта: `auto_check_list` и `auto_check_list_bot`). Создайте там `.env` на основе `.env.example` из любого из проектов и укажите `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` и при необходимости `DJANGO_API_URL`, `DJANGO_CALLBACK_ENDPOINT`.

```bash
git clone <url> && cd auto_check_list_bot
poetry install
# общий .env уже в родительской директории (puls/.env)
poetry run python main.py
```

Файл `.env` не попадает в репозиторий (см. `.gitignore`); для работы нужен общий `.env` в родительской директории обоих проектов.
...
- `.gitignore` — исключает из коммита `.env`, кэши Python, виртуальные окружения, артефакты сборки и тестов, служебные файлы IDE и ОС. Общий `.env` для Django и бота хранится в родительской директории обоих проектов.
